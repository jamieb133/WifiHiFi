...
CrossoverFilter::CrossoverFilter(filter_t type, double fCut, int taps)
{
    switch(type)
    {
        case LOWPASS: 
...
double CrossoverFilter::filter(double sample)
{
    if (m_fir != NULL)
    {
...
bool CrossoverFilter::HighpassCoeffs(int taps, double fCut)
{
...
bool CrossoverFilter::LowpassCoeffs(int taps, double fCut)
{
    m_coeffs = new double[taps]; 
